...
def _write_run_report_sh(out_dir: Path, python: str, result_dir: Path, invocation_cwd: Path) -> Path:
    """E2/N4: the third emitted `.sh` carries the same `cd` contract as the
    catbench jobgen runner units (verified from a DIFFERENT cwd — E3)."""
    ...
    lines = [
        ...
        "set -eu",
        f'cd "{invocation_cwd}"',
        f'exec "{python}" "{report_py}" --result "{result_dir}" --out "{out_dir}" --python "{python}"',
    ]
    sh_path.write_text("\n".join(lines) + "\n")
    sh_path.chmod(0o755)
    return sh_path
...
def main(argv: list[str] | None = None) -> int:
    ...
    sh_path = _write_run_report_sh(out_dir, sys.executable, result_dir, invocation_cwd)
