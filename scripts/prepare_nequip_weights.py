...
scripts/upstream_recipes.py (NequIP / Allegro fetch recipes): nequip-compile
downloads the nequip.net model and compiles it in one step.
...
#  zenodo package file name, zenodo md5). nequip.net resolves each URI to the
# zenodo record 18775904 (NequIP) / 16980200 (Allegro) file named here; the md5
# is zenodo's published checksum, used to accept a pre-downloaded local copy.
MODELS = {
    "nequip": [
        ("nequip.net:mir-group/NequIP-OAM-XL:0.1", "NequIP-OAM-XL", ["--modifiers", "enable_OpenEquivariance"],
         "NequIP-OAM-XL-0.1.nequip.zip", "3d2369c7238eb83a23141abdcb055a8f"),
        ("nequip.net:mir-group/NequIP-OAM-L:0.1", "NequIP-OAM-L", ["--modifiers", "enable_OpenEquivariance"],
         "NequIP-OAM-L-0.1.nequip.zip", "67144367c710a70a53a8e21acf331980"),
    ],
    ...
    "allegro": [
        ("nequip.net:mir-group/Allegro-OAM-L:0.1", "Allegro-OAM-L", ["--modifiers", "enable_CuEquivarianceContracter"],
         "Allegro-OAM-L-0.1.nequip.zip", "0db7f9b3c3a62e74d78b3fcf2973c462"),
    ],
}
...
def main(argv: list[str] | None = None) -> int:
    ...
    failed = 0
    for uri, version, extra, zip_name, md5 in MODELS[env_name]:
        out = out_dir / f"{version}_{arch}.nequip.pt2"
        source = (local_package(zip_dir, zip_name, md5) if zip_dir else None) or uri
