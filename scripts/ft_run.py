...
FAMILY_DATASET_TARGET = {
    ...
    # EquiformerV3: the prestage writes the ASE db files fairchem's ase_db dataset reads.
    "UMA": "extxyz", "EquiformerV3": "extxyz", "Nequix": "extxyz",
    # fairchemv1: the prestage writes the ASE .db files fairchem's ase_db dataset reads
    # from the canonical extxyz split.
    # EquFlash: the prestage writes the ASE db files GGNN's ase_db dataset reads.
    "fairchemv1": "extxyz", "EquFlash": "extxyz",
    "AlphaNet": "alphanet",
...
SEED_CONTROL = {
    ...
    "UMA": ("native", "job.seed + train_dataloader.batch_sampler_fn.seed, written into the fine-tune YAML "
                      "(fairchem-core configs/uma/finetune/uma_sm_finetune_template.yaml sampler seed; "
                      "job.seed is the fairchem job's global seed)"),
    ...
    "EquiformerV3": ("native", "fairchem --seed (the equiformer_v3 env's fairchem-core common/flags.py: seed for "
                               "torch, cuda, numpy)"),
...
# one itself) and proceed. oh-my-mlip does not redistribute these checkpoints
...
def build_mace(ctx: Context) -> CommandSpec:
    ...
    argv = [
        ...
        *( [f"--valid_file={valid_path}"] if valid_path else ["--valid_fraction=0.1"] ),
        ...
        f"--seed={int(ctx.seed)}",
        *(["--ema"] if ema is True or str(ema).lower() == "true" else []),  # store_true flag: no value
...
for dotted, value in json.loads(PATCH.read_text()).items():
    ...
    for p in parts[:-1]:
        node = node.setdefault(p, {{}})
    if value is None:
        node.pop(parts[-1], None)   # null in the patch removes a preset key
    else:
        node[parts[-1]] = value
...
def sevennet_patch(ctx: Context) -> dict:
    ...
    if modality:
        ...
        # the loader name is the key minus `load_`/`_path`, train/modal_dataset.py
        # from_config), so the preset key is dropped and the set is loaded as `validset`.
        patch[f"data.load_{modality}_validset_path"] = None
        if valid:
            patch["data.load_validset_path"] = _sevenn_modal_path(modality, str(valid))
    else:
...
# DPA4's SeZM descriptor raises NotImplementedError from change_type_map
# (deepmd/pt/model/descriptor/sezm.py), which `dp train --finetune` calls
# whenever input.json's type_map differs from the checkpoint's
# (deepmd/pt/train/training.py, finetune_rule_single.get_finetune_tmap()).
# The prestage therefore copies the checkpoint's own type_map into input.json,
# read the way deepmd/pt/utils/finetune.py get_finetune_rules reads it.
_DPA4_PRESTAGE = '''\
"""Set input.json's type_map to the foundation checkpoint's own type_map for a
DPA4 fine-tune. Generated by scripts/ft_run.py; runs inside the DPA4 env
before `dp --pt train`."""
import json
import pathlib
...
if "model_dict" in params:
    params = next(iter(params["model_dict"].values()))   # dp's default branch choice
type_map = list(params["type_map"])
cfg = json.loads(CONFIG.read_text())
missing = sorted(set(cfg["model"]["type_map"]) - set(type_map))
if missing:
...
def build_deepmd(ctx: Context) -> CommandSpec:
    ...
    if dpa4:
        prestage_path = ctx.out / "dpa4_prestage.py"
        return CommandSpec(
            argv=argv, config_path=config_path, config_text=config_text,
            pre_steps=[[ctx.resolved["python"], str(prestage_path)]],
            extra_files={prestage_path: _DPA4_PRESTAGE.format(ckpt=str(ckpt_for_cli), config=str(config_path))},
        )
...
# ── UMA (fairchem-core, `fairchem -c uma_sm_finetune_template.yaml`) ──────────
# Source: fairchem-core src/fairchem/core/scripts/create_uma_finetune_dataset.py
# (--train-dir/--val-dir of ase-readable files -> aselmdb shards, force RMS
# normalizer and per-element linear references stamped into the data YAML, plus
# the training YAML; TEMPLATE_DIR = Path("configs/uma/finetune") relative to the
# cwd) and configs/uma/finetune/{uma_sm_finetune_template.yaml,
# data/uma_conserving_data_task_*.yaml}. Those templates are not in the wheel,
# so the prestage fetches them from the fairchem repository at the installed
# version (the +g<sha> of a dev build, else tag fairchem_core-<version>) into a
# hub-owned cache and runs the generator with that cache as cwd. Output:
# job.run_dir/<timestamp_id>/checkpoints/final/inference_ckpt.pt, loaded with
# fairchem.core.units.mlip_unit.load_predict_unit (docs/core/common_tasks/fine_tuning.md).
_UMA_TEMPLATE_FILES = (
...
_UMA_PRESTAGE = '''\
"""Build the UMA fine-tune inputs with fairchem's own generator, then apply the
resolved settings. Generated by scripts/ft_run.py; runs inside the UMA env."""
import json
import pathlib
import re
import shutil
...
TRAIN = pathlib.Path({train!r})
VALID = pathlib.Path({valid!r})
CACHE = pathlib.Path({cache!r})
PATCH = pathlib.Path({patch!r})
FILES = {files!r}

version = fairchem.core.__version__
m = re.search(r"\\+g([0-9a-f]{{7,40}})", version)
ref = m.group(1) if m else "fairchem_core-" + version
root = CACHE / ref
for rel in FILES:
    dest = root / "configs" / "uma" / "finetune" / rel
    if dest.exists():
        continue
    url = "https://raw.githubusercontent.com/facebookresearch/fairchem/" + ref + "/configs/uma/finetune/" + rel
    dest.parent.mkdir(parents=True, exist_ok=True)
    try:
        with urllib.request.urlopen(url, timeout=60) as resp:
            dest.write_bytes(resp.read())
    except Exception as exc:
        raise SystemExit("[uma_prestage] could not fetch " + url + " for fairchem-core " + version + ": " + repr(exc))
print("[uma_prestage] templates for fairchem-core " + version + " at " + str(root))
...
for path in (src, data):
    if path.exists():
        shutil.rmtree(path)   # both are this run's own outputs; the generator requires a new dir
(src / "train").mkdir(parents=True)
(src / "val").mkdir(parents=True)
(src / "train" / TRAIN.name).symlink_to(TRAIN)
(src / "val" / VALID.name).symlink_to(VALID)

cmd = [sys.executable, "-m", "fairchem.core.scripts.create_uma_finetune_dataset",
       "--train-dir", str(src / "train"), "--val-dir", str(src / "val"), "--output-dir", str(data),
       "--uma-task", patch["uma_task"], "--regression-tasks", patch["regression_tasks"],
       "--base-model", patch["base_model"], "--num-workers", str(patch["num_workers"])]
subprocess.run(cmd, cwd=root, check=True)


def set_dotted(node, dotted, value):
    parts = dotted.split(".")
    for p in parts[:-1]:
        node = node.setdefault(p, {{}})
    node[parts[-1]] = value
...
train_yaml = data / "uma_sm_finetune_template.yaml"
cfg = yaml.safe_load(train_yaml.read_text())
for dotted, value in patch["train_yaml"].items():
    set_dotted(cfg, dotted, value)
train_yaml.write_text(yaml.safe_dump(cfg, sort_keys=False))

data_yaml = next((data / "data").glob("*.yaml"))
dcfg = yaml.safe_load(data_yaml.read_text())
for task in dcfg["tasks_list"]:
    coef = patch["loss_coefficients"].get(task["name"])
    if coef is not None and "loss_fn" in task:
        task["loss_fn"]["coefficient"] = coef
data_yaml.write_text(yaml.safe_dump(dcfg, sort_keys=False))
print("[uma_prestage] task=" + patch["uma_task"] + " regression=" + patch["regression_tasks"] + " -> " + str(train_yaml))
'''


def build_uma(ctx: Context) -> CommandSpec:
    task = (ctx.finetune.get("variant_args") or {}).get("--uma-task")
    s = ctx.settings
    task = s.get("--uma-task") if ctx.settings_origins.get("--uma-task") == "user" else task
    valid_tasks = ("omol", "omat", "odac", "oc20", "oc25", "omc")
    if task not in valid_tasks:
        raise ValueError(f"UMA: task {task!r} is not a fine-tunable UMATask {valid_tasks} "
                         "(fairchem-core create_uma_finetune_dataset.py --uma-task choices)")
    base_model = resolve_foundation_checkpoint(ctx.version, ctx.resolved)
    valid = ctx.dataset_paths.get("valid")
    if not valid:
        raise ValueError("UMA: the generator needs a separate validation set; use --split below 1.0")

    if ctx.settings_origins.get("--regression-tasks") == "user":
        regression = s["--regression-tasks"]
    else:
        # stress is trained only when asked for (--include-stress, or a user stress weight > 0)
        stress_user = ctx.settings_origins.get("stress loss coefficient") == "user"
        regression = "efs" if stress_user and float(s.get("stress loss coefficient") or 0) > 0 else "ef"

    steps = s.get("steps")
    train_yaml = {
        "base_model_name": base_model,
        "epochs": None if steps is not None else s.get("epochs"),
        "steps": steps,
        "batch_size": s.get("batch_size"),
        "lr": s.get("lr"),
        "weight_decay": s.get("weight_decay"),
        "max_neighbors": s.get("max_neighbors"),
        "evaluate_every_n_steps": s.get("evaluate_every_n_steps"),
        "checkpoint_every_n_steps": s.get("checkpoint_every_n_steps"),
        "runner.train_eval_unit.cosine_lr_scheduler_fn.warmup_factor": s.get("cosine_lr_scheduler_fn.warmup_factor"),
        "runner.train_eval_unit.cosine_lr_scheduler_fn.warmup_epochs": s.get("cosine_lr_scheduler_fn.warmup_epochs"),
        "runner.train_eval_unit.cosine_lr_scheduler_fn.lr_min_factor": s.get("cosine_lr_scheduler_fn.lr_min_factor"),
        "runner.train_eval_unit.clip_grad_norm": s.get("clip_grad_norm"),
        "job.run_dir": str(ctx.out / "runs"),
        "job.timestamp_id": "ft",
        "job.device_type": "CUDA" if ctx.device == "cuda" else "CPU",
        "job.seed": int(ctx.seed),
        "train_dataloader.batch_sampler_fn.seed": int(ctx.seed),
    }
    for name, dotted in (("ema_decay", "runner.train_eval_unit.ema_decay"), ("bf16", "runner.train_eval_unit.bf16")):
        if ctx.settings_origins.get(name) == "user":   # not in the template; upstream code default otherwise
            train_yaml[dotted] = s[name]
    if steps is None and s.get("epochs") is None:
        raise ValueError("UMA: training length is not set; pass --epochs or --max-steps")
    for key in ("batch_size", "lr"):
        if train_yaml[key] is None:
            raise ValueError(f"UMA: {key} has no value; pass it")

    loss = {"energy": s.get("energy loss coefficient"), "forces": s.get("forces loss coefficient")}
    if regression == "efs":
        loss["stress"] = s.get("stress loss coefficient")
    patch = {
        "uma_task": task, "regression_tasks": regression, "base_model": base_model,
        "num_workers": int(s.get("--num-workers") or 8),
        "train_yaml": {k: v for k, v in train_yaml.items() if v is not None or k in ("epochs", "steps")},
        "loss_coefficients": {k: float(v) for k, v in loss.items() if v is not None},
    }
    patch_path = ctx.out / "uma_patch.json"
    prestage_path = ctx.out / "uma_prestage.py"
    prestage_text = _UMA_PRESTAGE.format(
        out=str(ctx.out), train=str(ctx.dataset_paths["train"]), valid=str(valid),
        cache=str(Path(reg.home()) / "models" / ctx.resolved["env"] / "finetune_templates"),
        patch=str(patch_path), files=list(_UMA_TEMPLATE_FILES),
    )
    argv = [entrypoint_bin(ctx.resolved, "fairchem"), "-c", str(ctx.out / "uma_data" / "uma_sm_finetune_template.yaml")]
    return CommandSpec(
        argv=argv, config_path=patch_path, config_text=json.dumps(patch, indent=2) + "\n",
        pre_steps=[[ctx.resolved["python"], str(prestage_path)]],
        extra_files={prestage_path: prestage_text},
    )


# ── fairchemv1 / eSEN (fairchem-core 1.10, `fairchem --mode train --checkpoint`) ─
# Source: fairchem-core 1.10.0 docs/core/common_tasks/fine-tuning/fine-tuning.md
# (generate_yml_config from the checkpoint's own config, then --mode train
# --checkpoint: TrainTask.setup -> load_checkpoint loads weights, normalizers,
# element references and EMA; the released esen_30m_oam.pt carries no
# optimizer/step state, so this starts a new run), common/flags.py (--run-dir,
# --timestamp-id, --seed, --cpu), trainers/base_trainer.py (checkpoints under
# <run-dir>/checkpoints/<timestamp_id>/, checkpoint.pt written every epoch),
# models/esen/trainers/trainer.py load_extras (every scheduler_params *epochs*
# key is multiplied by iterations per epoch, so the checkpoint's own
# warmup_epochs: 8527 -- already a step count -- must be replaced by a fraction)
# and datasets/ase_datasets.py (format ase_db, a2g_args r_energy/r_forces/r_stress).
# The settings file's official values are the eqV2 OMat24 fine-tune template's;
# they set the optimizer here, while the loss coefficients stay the checkpoint's
# own (the eqV2 isotropic/anisotropic stress terms do not exist for eSEN) unless
# the user sets them.
_ESEN_PRESTAGE = '''\
"""Write the ASE db files and the eSEN fine-tune config from the checkpoint's own
config. Generated by scripts/ft_run.py; runs inside the fairchemv1 env."""
import json
import pathlib
...
for split, key in (("train", "dataset.train"), ("val", "dataset.val")):
    ...
    update[key + ".a2g_args.r_energy"] = True
    update[key + ".a2g_args.r_forces"] = True
    update[key + ".a2g_args.r_stress"] = patch["stress"]
update["logger"] = "tensorboard"
generate_yml_config(patch["checkpoint"], str(CONFIG),
                    delete=["slurm", "cmd", "logger", "task", "dataset", "val_dataset", "test_dataset", "relax_dataset"],
                    update=update)

cfg = yaml.safe_load(CONFIG.read_text())
losses = []
for entry in cfg["loss_functions"]:
    (name, spec), = entry.items()
    if name == "stress" and not patch["stress"]:
        continue
    if patch["loss"].get(name) is not None:
        spec["coefficient"] = patch["loss"][name]
    losses.append({{name: spec}})
cfg["loss_functions"] = losses
if not patch["stress"]:
    cfg["outputs"].pop("stress", None)
    cfg.get("evaluation_metrics", {{}}).get("metrics", {{}}).pop("stress", None)
CONFIG.write_text(yaml.safe_dump(cfg, sort_keys=False))
print("[esen_prestage] stress=" + str(patch["stress"]) + " losses=" + json.dumps(losses) + " -> " + str(CONFIG))
'''


def build_fairchemv1(ctx: Context) -> CommandSpec:
    s, origins = ctx.settings, ctx.settings_origins
    checkpoint = os.path.expandvars(resolve_foundation_checkpoint(ctx.version, ctx.resolved))
    valid = ctx.dataset_paths.get("valid")
    if not valid:
        raise ValueError("fairchemv1: the trainer needs a validation set; use --split below 1.0")
    if s.get("optim.max_epochs") is None:
        raise ValueError("fairchemv1: training length is not set; pass --epochs")
    if s.get("optim.lr_initial") is None:
        raise ValueError("fairchemv1: learning rate has no value; pass --lr")
    stress = bool(s.get("dataset.train.a2g_args.r_stress")) and origins.get("dataset.train.a2g_args.r_stress") == "user"

    def number(name):
        value = s.get(name)
        return value if isinstance(value, (int, float)) and not isinstance(value, bool) else None

    batch = number("optim.batch_size")
    update = {
        "optim.max_epochs": number("optim.max_epochs"),
        "optim.batch_size": batch,
        # upstream default is "optim.batch_size" (the same number)
        "optim.eval_batch_size": number("optim.eval_batch_size") or batch,
        "optim.lr_initial": number("optim.lr_initial"),
        "optim.optimizer_params.weight_decay": number("optim.optimizer_params.weight_decay"),
        "optim.scheduler_params.warmup_factor": number("optim.scheduler_params.warmup_factor"),
        "optim.scheduler_params.warmup_epochs": number("optim.scheduler_params.warmup_epochs"),
        "optim.scheduler_params.lr_min_factor": number("optim.scheduler_params.lr_min_factor"),
        "optim.clip_grad_norm": number("optim.clip_grad_norm"),
        "optim.ema_decay": number("optim.ema_decay"),
        "optim.eval_every": number("optim.eval_every"),
        "optim.num_workers": number("optim.num_workers"),
    }
    loss_names = {"energy": "loss_functions[energy].coefficient", "forces": "loss_functions[forces].coefficient",
                  "stress": "loss_functions[stress*].coefficient"}
    patch = {
        "checkpoint": checkpoint, "train": str(ctx.dataset_paths["train"]), "valid": str(valid), "stress": stress,
        "update": {k: v for k, v in update.items() if v is not None},
        "loss": {k: float(s[n]) for k, n in loss_names.items() if origins.get(n) == "user" and s.get(n) is not None},
    }
    patch_path = ctx.out / "esen_patch.json"
    config_path = ctx.out / "config.yml"
    prestage_path = ctx.out / "esen_prestage.py"
    prestage_text = _ESEN_PRESTAGE.format(out=str(ctx.out), patch=str(patch_path), config=str(config_path))
    argv = [entrypoint_bin(ctx.resolved, "fairchem"), "--mode", "train", "--config-yml", str(config_path),
            "--checkpoint", checkpoint, "--run-dir", str(ctx.out / "runs"), "--timestamp-id", "ft",
            "--seed", str(int(ctx.seed))]
    if ctx.device == "cpu":
        argv.append("--cpu")
    if origins.get("--amp") == "user" and s.get("--amp"):
        argv.append("--amp")
    return CommandSpec(
        argv=argv, config_path=patch_path, config_text=json.dumps(patch, indent=2) + "\n",
        pre_steps=[[ctx.resolved["python"], str(prestage_path)]],
...
# ── EquFlash (GGNN, `python -m GGNN.main --mode train --config-yml`) ──────────
# Source: GGNN (SamsungDS, pinned 16b5cae) configs/equflash/equflash.yml, the
# fine-tune template: task.finetune {checkpoint, reset_head}, task.strict_load,
# trainer: default, logger: files, dataset.{train,val} {format: ase_db, src,
# a2g_args}, OneCycleLR with total_iters: max_epochs. GGNN/trainer/trainer.py
# load_model (task.finetune.checkpoint loads model weights only; reset_head drops
# atomic_reduce.linear_out), load_extras (total_iters "max_epochs" -> steps),
# cmd.checkpoint_dir <run-dir>/<timestamp_id>/checkpoints, GGNN/_cli.py (fairchem
# 1.10 flags) and GGNN/common/utils.py new_trainer_context (trainer "default" is
# GGNN's own Trainer). configs/ is not in the installed package, so the model,
# outputs and losses come from the checkpoint's own config (both released
# checkpoints embed it); the template supplies the fine-tune keys above.
_EQUFLASH_PRESTAGE = '''\
"""Write the ASE db files and the EquFlash fine-tune config from the checkpoint's
own config. Generated by scripts/ft_run.py; runs inside the equflash env."""
import json
import pathlib
...
cfg = torch.load(patch["checkpoint"], map_location="cpu", weights_only=False)["config"]

dataset = {{}}
for split, src in (("train", patch["train"]), ("val", patch["valid"])):
    db = OUT / "equflash_data" / (split + ".db")
    ...
    with connect(db) as con:
        for atoms in read(src, ":"):
            con.write(atoms)
    dataset[split] = {{"format": "ase_db", "src": [str(db)],
                      "a2g_args": {{"r_energy": True, "r_forces": True, "r_stress": patch["stress"]}}}}

for key in ("cmd", "slurm", "val_dataset", "test_dataset", "relax_dataset"):
    cfg.pop(key, None)
cfg["trainer"] = "default"
cfg["logger"] = "files"
cfg["dataset"] = dataset
cfg["task"] = {{"finetune": {{"checkpoint": patch["checkpoint"], "reset_head": patch["reset_head"]}},
               "strict_load": patch["strict_load"]}}
optim = cfg["optim"]
for key, value in patch["optim"].items():
    if key == "weight_decay":
        optim.setdefault("optimizer_params", {{}})["weight_decay"] = value
    else:
        optim[key] = value
optim["total_iters"] = "max_epochs"   # GGNN converts it to steps for this run
optim.pop("total_steps", None)
...
if not patch["stress"]:
    ...
    cfg.get("evaluation_metrics", {{}}).get("metrics", {{}}).pop("stress", None)
CONFIG.write_text(yaml.safe_dump(cfg, sort_keys=False))
print("[equflash_prestage] model=" + str(cfg["model"].get("name")) + " stress=" + str(patch["stress"]) + " -> " + str(CONFIG))
'''


def build_equflash(ctx: Context) -> CommandSpec:
    s, origins = ctx.settings, ctx.settings_origins
    checkpoint = os.path.expandvars(resolve_foundation_checkpoint(ctx.version, ctx.resolved))
    valid = ctx.dataset_paths.get("valid")
    if not valid:
        raise ValueError("EquFlash: the trainer needs a validation set; use --split below 1.0")
    for name in ("max_epochs", "lr_initial", "batch_size"):
        if s.get(name) is None:
            raise ValueError(f"EquFlash: {name} has no value; pass it")

    def number(name):
        value = s.get(name)
        return value if isinstance(value, (int, float)) and not isinstance(value, bool) else None

    optim = {name: number(name) for name in (
        "batch_size", "eval_batch_size", "max_atoms", "num_workers", "lr_initial", "max_lr", "weight_decay",
        "start_factor", "end_factor", "pct_start", "max_epochs", "ema_decay", "eval_every")}
    stress = origins.get("stress_coefficient") == "user" and float(s.get("stress_coefficient") or 0) > 0
    loss = {"energy": number("energy_coefficient"), "forces": number("forces_coefficient")}
    if stress:
        loss["stress"] = number("stress_coefficient")
    patch = {
        "checkpoint": checkpoint, "train": str(ctx.dataset_paths["train"]), "valid": str(valid), "stress": stress,
    ...
    patch_path = ctx.out / "equflash_patch.json"
    config_path = ctx.out / "config.yml"
    prestage_path = ctx.out / "equflash_prestage.py"
    prestage_text = _EQUFLASH_PRESTAGE.format(out=str(ctx.out), patch=str(patch_path), config=str(config_path))
    argv = [ctx.resolved["python"], "-m", "GGNN.main", "--mode", "train", "--config-yml", str(config_path),
            "--run-dir", str(ctx.out / "runs"), "--timestamp-id", "ft", "--seed", str(int(ctx.seed))]
    if ctx.device == "cpu":
        argv.append("--cpu")
    return CommandSpec(
        argv=argv, config_path=patch_path, config_text=json.dumps(patch, indent=2) + "\n",
...
# ── Nequix (nequix 0.4.3, `nequix_train config.yml`, JAX) ────────────────────
# Source: nequix 0.4.3 nequix/train.py (main takes one config path;
# finetune_from loads the pretrained model when the file exists and raises
# NotImplementedError if atom_energies is also set; stats come from the config
# when shift/scale/avg_n_* are present; config["kernel"] is required;
# checkpoint.nqx is saved under wandb.run.dir whenever the validation loss
# improves; state_path keeps the training state), nequix/model.py save_model
# (the .nqx file's first line is the JSON training config, so the architecture
# keys come from the checkpoint itself) and nequix/data.py AseDBDataset (a
# single ASE db file or a directory of .aselmdb). The official fine-tune config
# is configs/nequix-oam-1.yml (repo-only); its optimizer/loss values are the
# settings file's official values. kernel: true needs OpenEquivariance with JAX
# support; unless the user sets it, it is used only when that package imports.
_NEQUIX_PRESTAGE = '''\
"""Write the ASE db files and the Nequix fine-tune config from the .nqx header.
Generated by scripts/ft_run.py; runs inside the nequix env."""
...
for key in ("atom_energies", "resume_from", "valid_frac"):
    cfg.pop(key, None)
cfg["finetune_from"] = patch["checkpoint"]
cfg["train_path"] = paths["train"]
cfg["valid_path"] = paths["val"]
cfg["state_path"] = str(OUT / "state.pkl")
cfg["cache_dir"] = str(OUT / "cache")
cfg.update(patch["config"])

has_oeq = importlib.util.find_spec("openequivariance") is not None
kernel = patch["kernel"]
if kernel is None:
    kernel = has_oeq
elif kernel and not has_oeq:
    raise SystemExit("[nequix_prestage] kernel: true needs OpenEquivariance with JAX support, which this env "
                     "does not have; pass --set kernel=false")
cfg["kernel"] = bool(kernel)
CONFIG.write_text(yaml.safe_dump(cfg, sort_keys=False))
print("[nequix_prestage] kernel=" + str(cfg["kernel"]) + " stress_weight=" + str(cfg["stress_weight"]) + " -> " + str(CONFIG))
'''


# nequix's DataLoader forks 16 workers after the parent has opened the .aselmdb files
# (AseDBDataset.__init__ reads the ids). ase-db-backends' LMDBDatabase.env reopens the
# environment when the pid changes, and py-lmdb then refuses a path still open in the forked
# process image ("already open in this process"). The launcher closes the inherited handle
# before that reopen and then runs nequix's own trainer unchanged.
_NEQUIX_LAUNCHER = '''\
"""Run nequix.train.main with forked data-loader workers able to open the aselmdb files.
Generated by scripts/ft_run.py; runs inside the nequix env."""
import os
import sys

import ase_db_backends.aselmdb as aselmdb

_open = aselmdb.LMDBDatabase._open_lmdb_env


def _open_after_fork(self):
    env = getattr(self, "_env", None)
    if env is not None and getattr(self, "_env_pid", None) != os.getpid():
        try:
            env.close()   # the handle inherited from the parent process
        except Exception:
            pass
    _open(self)


aselmdb.LMDBDatabase._open_lmdb_env = _open_after_fork

from nequix.train import main  # noqa: E402

sys.argv = ["nequix_train", sys.argv[1]]
main()
'''


def build_nequix(ctx: Context) -> CommandSpec:
    s, origins = ctx.settings, ctx.settings_origins
    checkpoint = os.path.expandvars(resolve_foundation_checkpoint(ctx.version, ctx.resolved))
    valid = ctx.dataset_paths.get("valid")
    if not valid:
        raise ValueError("Nequix: a validation set is required; use --split below 1.0")
    for name in ("n_epochs", "batch_size", "learning_rate"):
        if s.get(name) is None:
            raise ValueError(f"Nequix: {name} has no value; pass it")

    def number(name):
        value = s.get(name)
        return value if isinstance(value, (int, float)) and not isinstance(value, bool) else None

    stress = origins.get("stress_weight") == "user" and float(s.get("stress_weight") or 0) > 0
    config = {name: number(name) for name in (
        "n_epochs", "batch_size", "learning_rate", "warmup_epochs", "warmup_factor", "grad_clip_norm",
        "weight_decay", "energy_weight", "force_weight", "ema_decay", "log_every")}
    config["stress_weight"] = float(s["stress_weight"]) if stress else 0.0   # stress trained only when asked for
    for name in ("optimizer", "loss_type"):
        if isinstance(s.get(name), str):
            config[name] = s[name]
    patch = {
        "checkpoint": checkpoint, "train": str(ctx.dataset_paths["train"]), "valid": str(valid),
        "config": {k: v for k, v in config.items() if v is not None},
        "kernel": s.get("kernel") if origins.get("kernel") == "user" else None,
    }
    patch_path = ctx.out / "nequix_patch.json"
    config_path = ctx.out / "config.yml"
    prestage_path = ctx.out / "nequix_prestage.py"
    prestage_text = _NEQUIX_PRESTAGE.format(out=str(ctx.out), patch=str(patch_path), config=str(config_path))
    launcher_path = ctx.out / "nequix_launch.py"
    return CommandSpec(
        argv=[ctx.resolved["python"], str(launcher_path), str(config_path)],
        config_path=patch_path, config_text=json.dumps(patch, indent=2) + "\n",
        pre_steps=[[ctx.resolved["python"], str(prestage_path)]],
        extra_files={prestage_path: prestage_text, launcher_path: _NEQUIX_LAUNCHER},
        # checkpoint.nqx is written under wandb.run.dir; offline keeps it local
        extra_env={"WANDB_MODE": "offline", "WANDB_DIR": str(ctx.out)},
    )
...
# ── EquiformerV3 (source-derived; installed `fairchem --mode train`) ─────────
# Upstream documents no procedure for fine-tuning the released checkpoint on user data.
# This path is read from the installed source (the equiformer_v3 env's fairchem fork):
# fairchem/experimental/trainers/equiformer_v3_dens_trainer.py load_model
# (optim.load_pretrained_weights: weights-only, shape-checked load; optimizer, EMA and
# scheduler start fresh, unlike --checkpoint, which would resume the released
# checkpoint's epoch 2 / step 180280), load_extras (every scheduler_params *epochs* key
# is multiplied by iterations per epoch), use_denoising_pos off for fine-tuning (the
# README's gradient fine-tuning setting), core/modules/normalization/element_references.py
# create_element_references (a .pt state dict file) and core/datasets/base_dataset.py
# (metadata.npz natoms for optim.load_balancing: atoms). The model, outputs, losses and
# normalizers come from the checkpoint's own config; its element references are saved to
# a local file because the config points at the authors' .npz. Optimizer values are the
# settings file's (the eqV2 OMat24 fine-tune template); the loss coefficients stay the
# checkpoint's own unless the user sets them (the template's isotropic/anisotropic stress
# terms do not exist in this model).
_EQV3_PRESTAGE = '''\
"""Write the EquiformerV3 fine-tune config from the checkpoint's own config.
Generated by scripts/ft_run.py; runs inside the equiformer_v3 env."""
import json
import pathlib
...
ckpt = torch.load(patch["checkpoint"], map_location="cpu", weights_only=False)
cfg = ckpt["config"]
data_dir = OUT / "eqv3_data"
data_dir.mkdir(parents=True, exist_ok=True)
refs = data_dir / "element_references.pt"
torch.save(ckpt["elementrefs"]["energy"], refs)
del ckpt
...
transforms = dict((cfg.get("dataset") or {{}}).get("transforms") or {{}})
transforms["element_references"] = {{"energy": {{"file": str(refs)}}}}
cfg["dataset"] = {{
    split: {{"format": "ase_db", "src": [dbs[split]], "transforms": transforms,
             "a2g_args": {{"r_energy": True, "r_forces": True, "r_stress": stress, "r_edges": False, "r_pbc": True}}}}
...
for key, value in patch["optim"].items():
    if key == "weight_decay":
        ...
        optim.setdefault("scheduler_params", {{}})[key] = value
    else:
        optim[key] = value
iters = max(1, -(-n_train // int(optim.get("batch_size") or 1)))
warmup = optim.get("scheduler_params", {{}}).get("warmup_epochs")
if isinstance(warmup, (int, float)) and int(warmup * iters) < 1:
    optim["scheduler_params"]["warmup_epochs"] = (1.0 + 1e-9) / iters
    print("[eqv3_prestage] warmup_epochs " + str(warmup) + " is < 1 step at " + str(iters) + " iterations/epoch; using one step")

losses = []
for entry in cfg["loss_functions"]:
    (name, spec), = entry.items()
    if name == "stress" and not stress:
        continue
    if patch["loss"].get(name) is not None:
        spec["coefficient"] = patch["loss"][name]
    losses.append({{name: spec}})
...
if not stress:
    ...
    cfg.get("evaluation_metrics", {{}}).get("metrics", {{}}).pop("stress", None)
CONFIG.write_text(yaml.safe_dump(cfg, sort_keys=False))
print("[eqv3_prestage] stress=" + str(stress) + " losses=" + json.dumps(losses) + " -> " + str(CONFIG))
'''


def build_equiformerv3(ctx: Context) -> CommandSpec:
    s, origins = ctx.settings, ctx.settings_origins
    checkpoint = os.path.expandvars(resolve_foundation_checkpoint(ctx.version, ctx.resolved))
    valid = ctx.dataset_paths.get("valid")
    if not valid:
        raise ValueError("EquiformerV3: the trainer needs a validation set; use --split below 1.0")
    for name in ("max_epochs", "lr_initial", "batch_size"):
        if s.get(name) is None:
            raise ValueError(f"EquiformerV3: {name} has no value; pass it")
    ...
    optim = {name: number(name) for name in (
        "batch_size", "eval_batch_size", "num_workers", "lr_initial", "weight_decay", "warmup_factor",
        "warmup_epochs", "lr_min_factor", "max_epochs", "clip_grad_norm", "ema_decay", "eval_every")}
    if isinstance(s.get("load_balancing"), str):
        optim["load_balancing"] = s["load_balancing"]
    stress = (origins.get("stress_isotropic_coefficient") == "user"
              and float(s.get("stress_isotropic_coefficient") or 0) > 0)
    loss = {}
    for name, setting in (("energy", "energy_coefficient"), ("forces", "forces_coefficient")):
        if origins.get(setting) == "user" and s.get(setting) is not None:
    ...
    config_path = ctx.out / "config.yml"
    prestage_path = ctx.out / "eqv3_prestage.py"
    prestage_text = _EQV3_PRESTAGE.format(out=str(ctx.out), patch=str(patch_path), config=str(config_path))
    argv = [entrypoint_bin(ctx.resolved, "fairchem"), "--mode", "train", "--config-yml", str(config_path),
            "--run-dir", str(ctx.out / "runs"), "--timestamp-id", "ft", "--seed", str(int(ctx.seed))]
    if ctx.device == "cpu":
        argv.append("--cpu")
...
BUILDERS = {
    "EquiformerV3": build_equiformerv3,
    "ORB": build_orb,
    "Nequix": build_nequix,
