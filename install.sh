...
#
#   OMM_USE_LOCK=1 ./install.sh <env>
#                exact replay instead of the recipe: conda create from
#                envs/locks/<env>.conda.txt (@EXPLICIT) then
#                pip install --no-deps -r envs/locks/<env>.pip.txt. Locks are
#                generated from a passing fresh-root campaign by
#                scripts/gen_env_lock.py. A missing lock fails the env.
#
...
CATBENCH_PIN="${OMM_CATBENCH_VERSION:-1.1.4}"
...
install_one() {
  ...
  if [ "$skip_build" -eq 0 ]; then
    ...
    if [ "${OMM_USE_LOCK:-0}" = "1" ]; then
      # Exact replay: the conda @EXPLICIT package list, then the pip set with
      # --no-deps (no resolution), both generated by scripts/gen_env_lock.py
      # from a passing fresh-root campaign. A missing lock is a hard failure —
      # never a silent fall back to the (resolving) recipe.
      local lock_conda="$ENVS_DIR/locks/$env_name.conda.txt"
      local lock_pip="$ENVS_DIR/locks/$env_name.pip.txt"
      if [ ! -e "$lock_conda" ] || [ ! -e "$lock_pip" ]; then
        echo "  OMM_USE_LOCK=1 but no lock for '$env_name' ($lock_conda, $lock_pip) — no sentinel written." >&2
        return 1
      fi
      ...
      "$prefix/bin/pip" install --no-deps -r "$lock_pip" || {
        echo "  pip install from lock FAILED for '$env_name' — no sentinel written." >&2
        return 1
      }
    elif [ -e "$build_sidecar" ]; then
